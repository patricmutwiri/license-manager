...
- Existing data/schema approach: JPA entities with Hibernate `ddl-auto`; no migration framework found.
...
- Use JPA/Hibernate schema generation for this completion because no migration tool exists in the repository.
- Use secure random opaque license keys and signed offline JSON artifacts with HMAC-SHA256, configured by `LICENSE_SIGNING_SECRET`.
- Use an `X-Admin-Api-Key` header for admin API authorization because the repo has OAuth2 login but no role model.
...
- Added API-key-protected admin API and public runtime API.
...
- Added tests for platform lifecycle, activation, floating seats, heartbeat reclamation, offline artifacts, version bounds, and admin auth.
